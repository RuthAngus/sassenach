...
def corr_run(time, flux, flux_err, ID, max_factor, min_ph, min_lph, savedir,
             savestuff=False):
    ...
    max_psearch_len = len(lc_tab.flux) / max_factor
    ...
    if acf_per_pos[0] != -9999:
        ...
        ph = max(acf_per_height[:2])
        lph = lh1[x]
        print 'KID = ', x, 'PEAK HEIGHT = ', ph
        print 'LOCAL PEAK HEIGHT = ', lph
        ...
        if ph < min_ph or lph < min_lph: period[x] = np.nan
